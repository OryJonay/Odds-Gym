...
from ..utils.constants.football import (
    COUNTRIES,
    CSV_CACHE_PATH,
    CSV_URL,
    LEAGUES,
...
class FootballDataMixin(object):
    ...
    def _create_odds_dataframe(self, country, league, start, end):
        ...
        if start < 2010:
            raise ValueError(
                f"Start year must be greater than 2010, start year is {start}"
            )
        if start > end:
        ...
        odds = [
            "".join(odd)
            for odd in itertools.product(SITES, ["H", "D", "A"])
...
class FootballDataDailyEnv(FootballDataMixin, ThreeWaySoccerOddsEnv):
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        odds_dataframe.rename(
            {
                f"{columns.title()}H": "home",
                f"{columns.title()}D": "draw",
                f"{columns.title()}A": "away",
            },
            axis="columns",
            inplace=True,
        )
