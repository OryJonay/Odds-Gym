...
def test_attributes(tennis_percentage_env):
    assert tennis_percentage_env.observation_space == Box(
        low=1.0, high=float("Inf"), shape=(1, 2)
    )
    assert tennis_percentage_env.action_space == Box(
        low=numpy.array([-1] * 2), high=numpy.array([1.0] * 2)
    )
    assert numpy.array_equal(
        tennis_percentage_env.players,
        numpy.array(
            [["Berrettini M.", "Harris A."], ["Berankis R.", "Carballes Baena R."]]
        ),
    )
