...
def test_attributes(tennis_daily_env):
    ...
    assert tennis_daily_env.observation_space == Box(
    ...
    assert numpy.array_equal(
        tennis_daily_env.players.values,
        numpy.array(
            [
...
@pytest.mark.parametrize(
    ...
    [((_(0),), 0), ((_(1), _(1)), -2), ((_(3), _(3)), -1.16), ((_(2), _(2)), 0.84)],
)
...
def test_multiple_steps(tennis_daily_env):
    ...
    numpy.testing.assert_almost_equal(
        tennis_daily_env.balance, tennis_daily_env.starting_bank - 2 + 1.25 - 0.66
    )
    assert done
