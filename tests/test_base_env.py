...
def test_attributes(basic_env):
    ...
    assert basic_env.observation_space == Box(low=1.0, high=float("Inf"), shape=(1, 2))
...
def test_step_illegal_action(basic_env):
    basic_env.balance = 1
    odds, reward, done, *_ = basic_env.step(
        3
    )  # illegal - making a double when when the balance is 1
    assert reward == -2
    assert not done
    assert basic_env.current_step == 1
...
@pytest.mark.parametrize(
    ...
    [
        (0, numpy.array([[0, 1]], dtype=numpy.float64)),
