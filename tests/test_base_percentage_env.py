...
def test_attributes(basic_percentage_env):
    assert basic_percentage_env.action_space == Box(
        low=numpy.array([-1] * 2), high=numpy.array([1.0] * 2)
    )
    assert basic_percentage_env.observation_space == Box(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (numpy.array((0.25, 0.0)), 2.5),
        (numpy.array((0.0, 0.25)), -2.5),
