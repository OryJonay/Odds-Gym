...
def test_attributes(daily_bets_percentage_env):
    assert daily_bets_percentage_env.action_space == Box(
        low=-ones(shape=(4,)), high=ones(shape=(4,))
    )
    assert daily_bets_percentage_env.observation_space == Box(
    ...
    assert daily_bets_percentage_env.days[0] == datetime.today().date() - timedelta(
        days=1
    )
...
@pytest.mark.parametrize(
    ...
    [
        (0, array([[0.0, 0.0], [0.0, 0.0]]).reshape(4), 0, False),
        (1, array([[0.0, 0.0], [0.0, 0.0]]).reshape(4), 0, True),
        (0, array([[0.1, 0.0], [0.1, 0.0]]).reshape(4), 2, False),
        (1, array([[0.1, 0.0], [0.1, 0.0]]).reshape(4), 6, True),
        (0, array([[0.1, 0.0], [0.0, 0.1]]).reshape(4), -2, False),
        (1, array([[0.1, 0.0], [0.0, 0.1]]).reshape(4), 2, True),
        (0, array([[0.0, 0.1], [0.0, 0.1]]).reshape(4), 0, False),
        (1, array([[0.0, 0.1], [0.0, 0.1]]).reshape(4), -2, True),
        (0, array([[0.1, 0.1], [0.1, 0.1]]).reshape(4), 2, False),
        (1, array([[0.1, 0.1], [0.1, 0.1]]).reshape(4), 4, True),
    ],
)
def test_step(
    daily_bets_percentage_env, current_step, action, expected_reward, finished
):
...
def test_attributes_of_non_uniform(daily_bets_percentage_env_non_uniform):
    assert daily_bets_percentage_env_non_uniform.action_space == Box(
        low=-ones(shape=(6,)), high=ones(shape=(6,))
    )
    assert daily_bets_percentage_env_non_uniform.observation_space == Box(
        low=0.0, high=float("Inf"), shape=(3, 2)
    )
    assert len(daily_bets_percentage_env_non_uniform.days) == 3
    assert daily_bets_percentage_env_non_uniform.days[
        0
...
def test_get_odds_non_uniform(
    daily_bets_percentage_env_non_uniform, current_step, excpected_odds
):
...
def test_get_bet_non_uniform(daily_bets_percentage_env_non_uniform, actions, bets):
    assert numpy.array_equal(
        daily_bets_percentage_env_non_uniform.get_bet(actions), bets
    )
...
def test_get_results_non_uniform(
    daily_bets_percentage_env_non_uniform, current_step, expected_results
):
    daily_bets_percentage_env_non_uniform.current_step = current_step
...
@pytest.mark.parametrize(
    "current_step,action,expected_reward,finished",
    [
        (0, array([0.0] * 6).reshape(2, 3).T.reshape(6), 0, False),
        (1, array([0.0] * 6).reshape(2, 3).T.reshape(6), 0, False),
        (2, array([0.0] * 6).reshape(2, 3).T.reshape(6), 0, True),
        (0, array([0.1] * 3 + [0.0] * 3).reshape(2, 3).T.reshape(6), -1, False),
        (1, array([0.1] * 3 + [0.0] * 3).reshape(2, 3).T.reshape(6), 3, False),
        (2, array([0.1] * 3 + [0.0] * 3).reshape(2, 3).T.reshape(6), 10, True),
...
def test_step_non_uniform(
    daily_bets_percentage_env_non_uniform,
    current_step,
    action,
    expected_reward,
    finished,
    ...
    daily_bets_percentage_env_non_uniform.current_step = current_step
    odds, reward, done, truncated, info = daily_bets_percentage_env_non_uniform.step(
        action
    )
    assert reward == expected_reward
    assert done == finished
...
@pytest.mark.parametrize(
    "current_step,action,expected_reward,finished",  # Initial bank = 10
...
def test_step_non_uniform_non_round_percentage_with_balance(
    daily_bets_percentage_env_non_uniform,
    current_step,
    action,
    expected_reward,
