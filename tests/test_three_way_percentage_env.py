...
def test_attributes(three_way_percentage_env):
    assert three_way_percentage_env.observation_space == Box(
        low=1.0, high=float("Inf"), shape=(1, 3)
    )
    assert three_way_percentage_env.action_space == Box(
        low=numpy.array([-1] * 3), high=numpy.array([1.0] * 3)
    )
    assert numpy.array_equal(
        three_way_percentage_env.teams, numpy.array([["FCB", "PSG"], ["MCB", "MTA"]])
    )
