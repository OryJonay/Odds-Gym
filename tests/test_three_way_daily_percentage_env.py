...
def test_attributes(three_way_daily_percentage_env):
    assert three_way_daily_percentage_env.action_space == Box(low=array([[-1] * 4] * 2).reshape(8),
                                                              high=array([[1] * 4] * 2).reshape(8))
...
@pytest.mark.parametrize("action,expected_reward", [(array([_(0), 0, 0, 0] * 2).reshape(8), 0),
                                                    (array([_(1), 0.1, 0, 0] * 2).reshape(8), 2),
                                                    (array([_(2), 0, 0.1, 0] * 2).reshape(8), 0),
                                                    (array([_(3), 0, 0, 0.1] * 2).reshape(8), -2),
                                                    (array([_(4), 0.1, 0.1, 0] * 2).reshape(8), 2),
                                                    (array([_(5), 0.1, 0, 0.1] * 2).reshape(8), 0),
                                                    (array([[_(6)] + [0] + [0.1] * 2] * 2).reshape(8), -2),
                                                    (array([[_(7)] + [0.1] * 3] * 2).reshape(8), 0)])
...
@pytest.mark.parametrize("current_step,action,expected_reward",
                         [(0, array([_(0), 0.1, 0.1, 0.1, _(0), 0.1, 0.1, 0.1]), 0),
                          (0, array([_(0), 0.1, 0.1, 0.1, _(0), 0.1, 0.1, 0.1, _(0), 0.1, 0.1, 0.1]), 0),
                          (0, array([_(2), 0.0, 0.25, 0.0, _(7), 0.2, 0.1, 0.1]), 6.5),
                          # Reward = (0.25*10*2)+(0.2*10*4)-0.25*10-0.2*10-0.1*10-0.1*10 = 6.5
                          (1, array([_(6), 0.0, 0.0, 0.0]), 0),
                          (1, array([_(6), 0.0, 0.4, 0.5]), 6),
                          # Reward = (0.5*10*3)-0.4*10-0.5*10 = 6
                          (2, array([_(2), 0.0, 0.15, 0.0, _(1), 0.135, 0, 0, _(7), 0.2, 0.2, 0.2]), 1),
                          # Reward = (0.15*10*3)+(0.135*10*1)+(0.2*10*2)-0.15*10-0.135*10-3*(0.2*10) = 1
                          (2, array([_(2), 0.0, 0.15, 0.0, _(1), 0.135, 0.135, 0.1, _(7), 0.4, 0.2, 0.2]), -10.85)
                          # Illegal bet
                          ])
...
def test_multiple_steps(three_way_daily_percentage_env):
    odds, reward, done, info = three_way_daily_percentage_env.step(array([_(3), 0, 0, 0.1] * 2).reshape(8))
    assert reward == -2
    ...
    odds, reward, done, info = three_way_daily_percentage_env.step(array([_(1), bet_size, 0, 0] * 2).reshape(8))
    assert reward == -2
    ...
    bet_size = 1 / three_way_daily_percentage_env.balance
    odds, reward, done, info = three_way_daily_percentage_env.step(array([_(2), 0, bet_size, 0] * 2).reshape(8))
    assert reward == -2
...
def test_multiple_steps_non_uniform(three_way_daily_percentage_env_non_uniform):
    ...
    odds, reward, done, info = three_way_daily_percentage_env_non_uniform.step(array([_(2), 0.0, 0.25, 0.0,
                                                                                      _(7), 0.2, 0.1, 0.1]))
    assert reward == 6.5
    ...
    current_bank += reward
    odds, reward, done, info = three_way_daily_percentage_env_non_uniform.step(array([_(2), 0.0, 0.4, 0.0]))
    ...
    current_bank += reward
    odds, reward, done, info = three_way_daily_percentage_env_non_uniform.step(array([_(2), 0.0, 0.15, 0.0,
                                                                                      _(1), 0.135, 0, 0,
                                                                                      _(7), 0.2, 0.2, 0.2]))
