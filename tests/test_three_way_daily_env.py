...
def test_attributes(three_way_daily_env):
    ...
    assert three_way_daily_env.observation_space == Box(
    ...
    assert numpy.array_equal(
        three_way_daily_env.bet_size_matrix, numpy.ones(shape=(2, 3))
    )
    assert numpy.array_equal(
        three_way_daily_env.teams.values,
        numpy.array(
            [
                ["FCB", "PSG"],
                ["MCB", "MTA"],
...
def test_step_non_uniform(
    three_way_daily_env_non_uniform, current_step, action, expected_reward
):
...
def test_multiple_steps_non_uniform(three_way_daily_env_non_uniform):
    ...
    odds, reward, done, truncated, info = three_way_daily_env_non_uniform.step(
        array([_(2), _(7)])
    )
