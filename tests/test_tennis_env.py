...
def test_attributes(tennis_env):
    ...
    assert tennis_env.observation_space == Box(low=1.0, high=float("Inf"), shape=(1, 2))
...
@pytest.mark.parametrize(
    "action,expected_reward", [(0, 0), (1, -1), (2, 0.11), (3, -0.89)]
)
...
def test_multiple_steps(tennis_env):
    ...
    numpy.testing.assert_almost_equal(
        tennis_env.balance, tennis_env.starting_bank - 1 + 0.73, 2
    )
    assert done
